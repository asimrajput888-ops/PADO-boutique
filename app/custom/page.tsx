...
export default function CustomPage() {
  ...
  const totalPrice = useMemo(() => {
    let price = 0;
    const garment = GARMENTS.find((g) => g.id === selection.garment);
    if (garment) price += garment.basePrice;

    const fabric = FABRICS.find((f) => f.id === selection.fabric);
  ...
  const cardVariants = {
    ...
    exit: { opacity: 0, rotateY: 15, scale: 0.95, transition: { duration: 0.3 } },
  };
    ...
    <div className="min-h-screen bg-[#FDFBF7] text-neutral-900 py-24 px-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-neutral-500">Step {step} of {totalSteps} — {step === 1 ? "Choose Garment" : step === 2 ? "Choose Fabric" : step === 3 ? "Choose Style" : step === 4 ? "Customize Details" : step === 5 ? "Your Fit" : "Final Preview"}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="hidden lg:flex flex-col justify-center items-center bg-white/40 backdrop-blur-sm border border-white/60 rounded-2xl p-8 shadow-xl relative h-[600px]">
            <div className="text-center">
              ...
              <div className="w-48 h-64 bg-neutral-200 rounded-lg flex items-center justify-center text-neutral-400 text-sm border border-neutral-300">
                {selection.garment ? "Suit Preview" : "Select a garment"}
              </div>
          ...
          <div className="perspective-1000">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                animate="animate"
                exit="exit"
                className="bg-white/80 backdrop-blur-xl border border-white/40 shadow-2xl rounded-2xl p-8 md:p-12"
              >
                {/* Step 1: Garment */}
                {step === 1 && (
                      ...
                      {GARMENTS.map((g) => (
                        <button
                          ...
                          className={`p-4 border rounded-lg text-left transition-all ${
                            selection.garment === g.id ? "border-amber-600 bg-amber-50" : "border-neutral-200 hover:border-amber-400"
                          }`}
                        >
                      ...
                      {FABRICS.map((f) => (
                        <button
                          ...
                          className={`w-full p-4 border rounded-lg text-left flex justify-between transition-all ${
                            selection.fabric === f.id ? "border-amber-600 bg-amber-50" : "border-neutral-200 hover:border-amber-400"
                          }`}
                        >
                    ...
                    <div className="space-y-3">
                      {FIT_OPTIONS.map((f) => (
                        <button
                          ...
                          className={`w-full p-4 border rounded-lg text-left flex justify-between transition-all ${
                            selection.fit === f.id ? "border-amber-600 bg-amber-50" : "border-neutral-200 hover:border-amber-400"
                          }`}
                        >
                          <span className="font-medium text-sm">{f.name}</span>
                          <span className="text-xs text-neutral-500">{f.price > 0 ? `+ Rs. ${f.price.toLocaleString()}` : "Included"}</span>
                ...
                {step === 6 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="bg-neutral-100 rounded-lg h-48 flex items-center justify-center text-neutral-400 text-sm">
                        Front / Back Preview
                      </div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="bg-neutral-50 p-3 rounded border border-neutral-200">
                          <p className="text-neutral-400 text-xs mb-1">Garment</p>
                          <p className="font-medium">{GARMENTS.find((g) => g.id === selection.garment)?.name || "Not selected"}</p>
                        </div>
                        <div className="bg-neutral-50 p-3 rounded border border-neutral-200">
                          <p className="text-neutral-400 text-xs mb-1">Fabric</p>
                          <p className="font-medium">{FABRICS.find((f) => f.id === selection.fabric)?.name || "Not selected"}</p>
                        </div>
                        <div className="bg-neutral-50 p-3 rounded border border-neutral-200">
                          <p className="text-neutral-400 text-xs mb-1">Style</p>
                          <p className="font-medium">{selection.style.length} options selected</p>
                        </div>
                        <div className="bg-neutral-50 p-3 rounded border border-neutral-200">
                          <p className="text-neutral-400 text-xs mb-1">Details</p>
                          <p className="font-medium">{selection.details.length} options selected</p>
                        </div>
                        <div className="bg-neutral-50 p-3 rounded border border-neutral-200 col-span-2">
                          <p className="text-neutral-400 text-xs mb-1">Fit</p>
                          <p className="font-medium">{FIT_OPTIONS.find((f) => f.id === selection.fit)?.name || "Not selected"}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-8 pt-6 border-t border-neutral-100">
                  ...
                  <div className="flex justify-between">
                    {step > 1 ? (
                      <button onClick={prevStep} className="text-neutral-500 hover:text-neutral-900 font-medium transition-colors">
                        &larr; Back
                      </button>
                    ) : <div />}
                    {step < totalSteps ? (
                      <button
                        ...
                        className="bg-neutral-900 text-white px-8 py-3 font-semibold hover:bg-amber-600 transition-all duration-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Next Step
                      </button>
                    ) : (
                      <button
                        ...
                        className="bg-amber-600 text-white px-8 py-3 font-semibold hover:bg-neutral-900 transition-all duration-300 rounded-lg"
                      >
                        Submit Order
                      </button>
                    )}
